...
#let code-block(
  ...
  #figure(
    ...
    gap: 1em,
    {
      ...
      set text(size: 9pt)
      if mono-font != none { set text(font: mono-font) }
  ...
  show raw.where(block: true): it => locate(loc => {
    ...
    if (is-diff) {
      if (cur-diff-code.before == none and cur-diff-code.after == none) {
        ...
        let code-block-content = grid(
          columns: if show-line-numbers { (auto, auto, 1fr) } else { (auto, 1fr) },
          inset: (y: 0.3em),
          align: left + top,
          stroke: none,
          ..diff-code
            ...
            .map(((index, line)) => {
              ...
              let line-number-content = grid.cell(
                  ...
                  text(
                    fill: line-number-color,
                    weight: if show-added or show-deleted { "bold" } else { "regular" },
                    if line-number == none { "" } else { str(line-number) }
                  )
                )
              ...
              let line-tag-content = grid.cell(
                fill: line-bg-color,
                inset: (
                  left: if show-line-numbers { 0em } else { 1.2em },
                  right: 0.5em
                ),
                text(
                  fill: line-number-color,
              ...
              if line.tag == "Spacer" {
                grid.cell(
                  fill: line-bg-color,
                  inset: (y: 1em),
                  colspan: if show-line-numbers { 3 } else { 2 },
                  {
                    let height = 9pt
                    let gap = 6pt
                    let stroke = (
